...
export default function CancelBookingModal({ booking, onClose, onConfirm }: Props) {
  const t = useT();
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('bookings.cancelFailed'));
      setSubmitting(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-surface border border-neutral-border rounded-xl p-5 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[15px] font-semibold text-neutral-primary">{t('bookings.cancelTitle')}</h2>
        ...
        <p className="text-[12px] text-neutral-secondary mb-3">{t('bookings.cancelRefundNote')}</p>

        <label className="block text-[12px] font-medium text-neutral-secondary mb-1.5">
          {t('bookings.reasonLabelOptional')}
        </label>
        <textarea
          value={reason}
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          >
            {submitting ? t('common.loading') : t('bookings.actionCancel')}
          </button>
        </div>
      </div>
    </div>
